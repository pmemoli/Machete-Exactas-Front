...
function parseResuelto(resuelto) {
    ...
    if (resFechaRegex !== null) {
        // Encontro una fecha en el nombre
        ...
        // No encontro una fecha en el nombre
        ...
        if (resYearRegex !== null) {
            ...
            // No hay ningun match para ordenar
        ...
        for (const cuatriRegex of cuatriRegexes) {
            ...
            if (resCuatriRegex !== null) {
                const esPrimero =
                    resCuatriRegex[0].includes('1') ||
                    resCuatriRegex[0].includes('Primer');
...
function sortResueltos(resueltos) {
    const resueltosParsed = resueltos.map((resuelto) => ({
        res: resuelto,
        tiempo: parseResuelto(resuelto.nombre),
    }));
...
export default function Resueltos({
    ...
    const resueltosDelTipo = resueltos.filter(
        (resuelto) => resuelto.tipoResuelto === tipoResuelto,
    );
    ...
    return (
        <div className="section">
            ...
            <ul className="items-resueltos">
                {resueltosSorted.map((resuelto) => (
                    <li>
                        <a
                            className="item-resuelto"
                            href={resuelto.link}
                            target="_blank"
                            rel="noreferrer"
                        >
